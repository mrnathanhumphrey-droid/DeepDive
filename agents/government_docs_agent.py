...
class GovernmentDocsAgent(BaseAgent):
    agent_type = AgentType.GOVERNMENT_DOCS
    system_prompt = load("agents/government_docs_system_prompt")

    def _call_claude(self, prompt: str, topic: str = "", sub_topic: str = "") -> str:
        """Override to use web search for verifying P.L. numbers, CFR parts, EO numbers."""
    ...
    def _get_analysis_instruction(self, mode: str = "general") -> str:
        if mode == "historical":
            return load("agents/government_docs_historical_analysis_instruction")
        return load("agents/government_docs_analysis_instruction")
