...
class CausalChainAgent(BaseAgent):
    """Traces the cause-and-effect chain from the historical anchor
    event forward through time to the present."""

    agent_type = AgentType.CAUSAL_CHAIN
    system_prompt = load("agents/causal_chain_system_prompt")
    ...
    def _get_analysis_instruction(self, mode: str = "general") -> str:
        return load("agents/causal_chain_analysis_instruction")
